...
## for CARR data analysis. Handles 9099 files                   ##
##################################################################
##################################################################
...
a = ""                  # Temo var for allegation data
...
directory_string = "/Users/Alexander/Desktop/CARR/9099/files"
...
for file in fileList:
    ...
    if file.endswith("t.txt"):  
        with open(file, errors='ignore') as f:
            for line in f:  
                if re.search("COMPLAINT CONTROL NUMBER:",line):
                    controlNum = line.split("NUMBER:",1)[1].split()
                elif re.search("FACILITY NAME:",line) and fnameCount == 0 and fnumCount == 0:   
                ...
                elif re.search("Estimated Days of Completion:",line):  
                    before = line.split("WITH:",1)[0].strip()
                    action = before.split("Estimated")[0].strip()

        dataList = [facilityName, facilityNum, phone, address, city, state, zipCode, visitType, metWith, date, beginT, endT, action]
    ...
    if file.endswith("t.txt"): 
        ...
        with open(file, errors='ignore') as f:
            for line in f:  
                if re.search("ALLEGATION(S):", line) and allegation_count == 0:
                    allegation_count += 1
                elif allegation_count > 0:
                    if line.isdigit():
                        allegation_count = 0
                    else:
                        a += (line.strip() + " ")
                elif re.search("DEFICIENCIES",line) and deficiency_count == 0:    
                    deficiency_count += 1
        ...
        with open("data_9099.csv", "a") as csv_file:
            writer = csv.writer(csv_file, delimiter=',')

            if rowCount == 0:
                header = ["Facility Name", "Facility Number", "Phone Number", "Address", "City", "State", "Zip Code", "Visit Type", "Met With", "Date", "Time Began", "Time Ended", "Action Taken"]
